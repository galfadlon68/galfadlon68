...
public class BinaryNode<T> {
    ...
    private String toString (int depth) {
        ...
        //first left
        if (left != null) {
            output = output + left.toString(depth + 1);
        }
        // second root
        output = output + space(depth) + data.toString() + "\n";
        // last right
        if (right != null) {
            output = output + right.toString(depth + 1);
        }
        return output;
    }
    //check number of space required
    private String space (int numOfSpace) {
        String space = "";
        for (int i = 0; i < 2 * numOfSpace; i++) {
            space = space + " ";
        }
        return space;
    }
